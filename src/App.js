...
var FIREBASE_PROJECT = "deespark-inventory";
var FIREBASE_KEY = "AIzaSyB46xvdqJkHK6Q4KFRd7GeYb8OmuRht_68";
var FIRESTORE_URL = "https://firestore.googleapis.com/v1/projects/" + FIREBASE_PROJECT + "/databases/(default)/documents/appdata/main?key=" + FIREBASE_KEY;
...
function toFirestore(obj) {
  function convert(val) {
    ...
    if (typeof val === "object") {
      var fields = {};
      Object.keys(val).forEach(function(k) { fields[k] = convert(val[k]); });
      return {mapValue: {fields: fields}};
    }
    return {stringValue: String(val)};
  }
  var fields = {};
  Object.keys(obj).forEach(function(k) { fields[k] = convert(obj[k]); });
  return {fields: fields};
}

/* Convert Firestore REST format back to JS object */
function fromFirestore(firestoreObj) {
  ...
  function convert(val) {
    ...
    if ("mapValue" in val) {
      ...
      Object.keys(fields).forEach(function(k) { obj[k] = convert(fields[k]); });
      return obj;
    }
    return null;
  }
  var result = {};
  var fields = firestoreObj.fields || {};
  Object.keys(fields).forEach(function(k) { result[k] = convert(fields[k]); });
  return result;
}
...
function loadFromFirestore(callback) {
  fetch(FIRESTORE_URL + "&fields=fields")
    ...
    .catch(function(e) { console.error("Load error:", e); callback(null); });
}
...
var todayStr = today.toLocaleDateString("en-NG",{day:"numeric",month:"short",year:"numeric"});
var todayISO = today.toISOString().slice(0,10);

function uid() { return Math.random().toString(36).slice(2,9); }
function deliveryRate(a) { return a.totalOrdersAssigned > 0 ? Math.round((a.totalOrdersDelivered / a.totalOrdersAssigned)*100) : 0; }
function totalStock(a) { return a.products.reduce((s,p)=>s+p.currentStock,0); }
function rc(r) { return r>=80?"#10b981":r>=60?"#f59e0b":"#ef4444"; }
function rb(r) { return r>=80?"#10b98120":r>=60?"#f59e0b20":"#ef444420"; }
function rl(r) { return r>=80?"Excellent":r>=60?"Average":"Poor"; }
function suggestQty(agent, products) {
  var rate = deliveryRate(agent);
  var mult = rate>=80?1.3:rate>=60?1.0:0.6;
  return products.map(function(p){
    var ap = agent.products.find(function(x){return x.id===p.id;});
    return Object.assign({}, p, {suggested: Math.max(10, Math.round(((ap&&ap.totalDelivered)||20)*mult))});
  });
}

/* ── SEED ADMIN ── */
/* Firebase handled via REST API above */

var INIT_STATE = {
  products: [],
  agents: [],
  staff: [{ id:"admin", username:"admin", password:"admin123", role:"admin", name:"Main Administrator", assignedAgents:[] }],
  pendingEntries: [],
  deleteRequests: [], // {id, type, targetId, targetName, requestedBy, requestedByName, requestedAt, status}
};

var DB_DOC = "appdata/main";
...
export default function App() {
  ...
  useEffect(()=>{
    try {
      var saved = localStorage.getItem("sp_session");
      if(saved){
  ...
  // Load data from Firestore REST API on startup + poll every 10s
  useEffect(()=>{
    function load() {
      loadFromFirestore(function(data) {
        if(data) {
          setState(function(prev) {
            return Object.assign({}, INIT_STATE, data, {
  ...
  // Save to Firestore via REST API whenever state changes
  ...
  var handleLogin = (s, remember=true) => {
    setSession(s);
    if(remember){
      try {
  ...
  var props = { state, update, session, setSession: handleLogout };
  var isAdmin = session.role==="admin" || session.role==="superadmin";
  return isAdmin ? <AdminApp {...props}/> : <StaffApp {...props}/>;
}
...
function Login({ staff, onLogin }) {
  ...
  var login = () => {
    var found = staff.find(s=>s.username===user.trim()&&s.password===pwd);
...
/* ══ SHARED UI ══════════════════════════════════════════════ */
function PageHeader({title,sub,right}) { return (
...
function StatCard({label,value,accent,icon}) { return (
...
function RateBar({rate,width=80}) { return (
...
function TH({children}) { return <th style={{textAlign:"left",fontSize:11,color:"#334155",fontWeight:700,letterSpacing:1.5,textTransform:"uppercase",padding:"10px 14px",borderBottom:"1px solid #151c2e",whiteSpace:"nowrap"}}>{children}</th>; }
function TD({children,style={}}) { return <td style={{padding:"11px 14px",fontSize:13,borderBottom:"1px solid #0a0c14",verticalAlign:"middle",...style}}>{children}</td>; }
function Inp({label,...props}) { return (
  <div>
...
function Sel({label,children,...props}) { return (
  <div>
    {label&&<label style={{fontSize:12,color:"#64748b",fontWeight:600,marginBottom:5,display:"block"}}>{label}</label>}
...
function Btn({children,onClick,variant="primary",small,style={}}) { return (
...
function InfoBox({type="info",children}) {
  var m={info:["#6366f1","#6366f112","#6366f125"],warn:["#f59e0b","#f59e0b12","#f59e0b25"],good:["#10b981","#10b98112","#10b98125"],danger:["#ef4444","#ef444412","#ef444425"]};
  var [c,bg,bd]=m[type];
...
function EmptyState({icon,title,sub,action}) { return (
...
function Shell({ session, setSession, nav, activeTab, setTab, children }) {
  ...
  var handleTab = (key) => { setTab(key); setSidebarOpen(false); };
      ...
      <aside className="desktop-sidebar" style={{width:collapsed?64:220,transition:"width .22s ease",background:"#0b0e18",borderRight:"1px solid #151c2e",display:"flex",flexDirection:"column",position:"sticky",top:0,height:"100vh",flexShrink:0,zIndex:100}}>
        ...
        {!collapsed&&(
          <div style={{padding:"12px 14px",borderTop:"1px solid #151c2e"}}>
            ...
            <div style={{fontSize:11,color:session.role==="admin"?"#ef4444":session.role==="superadmin"?"#818cf8":"#10b981",marginTop:1,fontWeight:700}}>{session.role==="admin"?"👑 Main Admin":session.role==="superadmin"?"⭐ Super Admin":"👤 Staff"}</div>
...
function AdminApp({ state, update, session, setSession }) {
  ...
  var { products, agents, staff, pendingEntries } = state;
  var pendingCount = pendingEntries.filter(e=>e.status==="pending").length;

  var totals = useMemo(()=>{
    var assigned  = agents.reduce((s,a)=>s+a.totalOrdersAssigned,0);
    var delivered = agents.reduce((s,a)=>s+a.totalOrdersDelivered,0);
    var stock     = agents.reduce((s,a)=>s+totalStock(a),0);
    var rate      = assigned>0?Math.round((delivered/assigned)*100):0;
    var critical  = agents.filter(a=>totalStock(a)<20&&a.products.length>0);
    var low       = agents.filter(a=>totalStock(a)>=20&&totalStock(a)<50&&a.products.length>0);
    return {assigned,delivered,stock,rate,critical,low};
  },[agents]);

  var addProduct    = p  => update(s=>({...s,products:[...s.products,p]}));
  var updateProduct = (id,fn) => update(s=>({...s,products:s.products.map(p=>p.id===id?fn(p):p)}));
  var deleteProduct = id => update(s=>({...s,products:s.products.filter(p=>p.id!==id),agents:s.agents.map(a=>({...a,products:a.products.filter(p=>p.id!==id)}))}));
  var addAgent      = a  => update(s=>({...s,agents:[...s.agents,a]}));
  var updateAgent   = (id,fn) => update(s=>({...s,agents:s.agents.map(a=>a.id===id?fn(a):a)}));
  var deleteAgent   = id => update(s=>({...s,agents:s.agents.filter(a=>a.id!==id),staff:s.staff.map(st=>({...st,assignedAgents:st.assignedAgents.filter(x=>x!==id)}))}));
  var addStaff      = st => update(s=>({...s,staff:[...s.staff,st]}));
  var updateStaff   = (id,fn) => update(s=>({...s,staff:s.staff.map(st=>st.id===id?fn(st):st)}));
  var deleteStaff   = id => update(s=>({...s,staff:s.staff.filter(st=>st.id!==id)}));
  var approveEntry  = id => update(s=>{
    var entry = s.pendingEntries.find(e=>e.id===id);
    if(!entry) return s;
    var newAgents = s.agents.map(a=>{
      ...
      return {
        ...
        products: a.products.map(p=>{
          var q=(entry.pQ||{})[p.id]?parseInt(entry.pQ[p.id])||0:0;
  ...
  var rejectEntry = id => update(s=>({...s,pendingEntries:s.pendingEntries.map(e=>e.id===id?{...e,status:"rejected"}:e)}));

  var isSuperAdmin = session.role === "superadmin";
  var deleteRequests = state.deleteRequests || [];
  var pendingDeletes = deleteRequests.filter(function(r){return r.status==="pending";}).length;
  ...
  var NAV = [
    ...
    ...(!isSuperAdmin ? [{key:"superadmins", label:"Super Admins", icon:"👑"}] : []),
    {key:"deletes", label: isSuperAdmin ? "My Delete Requests" : "Delete Requests", icon:"🗑", badge:pendingDeletes, badgeColor:"#ef4444"},
  ...
  // isSuperAdmin moved above NAV

  // Super admin delete request handlers
  var requestDelete = (type, targetId, targetName, onDone) => {
    update(s => {
      var existing = (s.deleteRequests||[]).find(r => r.targetId===targetId && r.status==="pending");
      if(existing) return s;
      return {...s, deleteRequests:[...(s.deleteRequests||[]), {
        id:uid(), type, targetId, targetName,
        requestedBy: session.id, requestedByName: session.name,
  ...
  var approveDelete = (reqId) => {
    var req = deleteRequests.find(r=>r.id===reqId);
    if(!req) return;
    // Execute the actual delete
    if(req.type==="agent")   deleteAgent(req.targetId);
    if(req.type==="product") deleteProduct(req.targetId);
    if(req.type==="staff")   deleteStaff(req.targetId);
    update(s=>({...s, deleteRequests:(s.deleteRequests||[]).map(r=>r.id===reqId?{...r,status:"approved"}:r)}));
  };

  var rejectDelete = (reqId) => {
    update(s=>({...s, deleteRequests:(s.deleteRequests||[]).map(r=>r.id===reqId?{...r,status:"rejected"}:r)}));
  };

  // Wrap delete functions — super admin must request, main admin deletes directly
  var handleDeleteAgent   = (id, name, onDone) => isSuperAdmin ? requestDelete("agent",   id, name||id, onDone) : deleteAgent(id);
  var handleDeleteProduct = (id, name, onDone) => isSuperAdmin ? requestDelete("product", id, name||id, onDone) : deleteProduct(id);
  var handleDeleteStaff   = (id, name, onDone) => isSuperAdmin ? requestDelete("staff",   id, name||id, onDone) : deleteStaff(id);

  var sharedProps = {state,products,agents,staff,totals,
    addProduct,updateProduct,deleteProduct:handleDeleteProduct,
    addAgent,updateAgent,deleteAgent:handleDeleteAgent,
    addStaff,updateStaff,deleteStaff:handleDeleteStaff,
    approveEntry,rejectEntry,pendingEntries,
    deleteRequests,approveDelete,rejectDelete,
    session,isSuperAdmin,update};
  ...
  return (
    <Shell session={session} setSession={setSession} nav={NAV} activeTab={tab} setTab={setTab} pendingCount={pendingCount}>
      ...
      {tab==="superadmins"  && !isSuperAdmin && <SuperAdminManager {...sharedProps}/>}
      {tab==="deletes"      && <DeleteRequests {...sharedProps}/>}
...
function StaffApp({ state, update, session, setSession }) {
  var [tab, setTab] = useState("entry");

  var myAgents = state.agents.filter(a=>session.assignedAgents.includes(a.id));
  var myPending = state.pendingEntries.filter(e=>e.staffId===session.id&&e.status==="pending").length;

  var submitEntry = entry => update(s=>({...s,pendingEntries:[...s.pendingEntries,{...entry,id:uid(),staffId:session.id,staffName:session.name,status:"pending",submittedAt:new Date().toISOString()}]}));
  ...
  return (
    <Shell session={session} setSession={setSession} nav={NAV} activeTab={tab} setTab={setTab}>
      {tab==="entry"    && <StaffEntry myAgents={myAgents} products={state.products} submitEntry={submitEntry} session={session}/>}
      ...
      {tab==="changepass" && <ChangePassword session={session} update={update} state={state}/>}
    </Shell>
  );
}
...
function AdminDashboard({agents,products,totals,pendingEntries,setTab}) {
  var ranked = [...agents].sort((a,b)=>deliveryRate(b)-deliveryRate(a));
  var pending = pendingEntries.filter(e=>e.status==="pending");
      ...
      <Card>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {products.map(p=>{
                var init=agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.initialStock||0);},0);
                var del =agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.totalDelivered||0);},0);
                var rem =agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.currentStock||0);},0);
                var pct =init>0?Math.round((rem/init)*100):0;
                return(
...
function Setup({products,agents,addProduct,updateProduct,deleteProduct,addAgent,updateAgent,deleteAgent}) {
  ...
  var [pName,setPName]=useState(""); const [pSku,setPSku]=useState("");
  var [pEdit,setPEdit]=useState(null); const [pEN,setPEN]=useState(""); const [pES,setPES]=useState("");
  var [aName,setAName]=useState(""); const [aState,setAState]=useState(""); const [aPhone,setAPhone]=useState("");
  var [aEdit,setAEdit]=useState(null); const [aEN,setAEN]=useState(""); const [aES,setAES]=useState(""); const [aEP,setAEP]=useState("");
  var [msg,setMsg]=useState(null);
  var flash=(text,type="good")=>{setMsg({text,type});setTimeout(()=>setMsg(null),3500);};
  ...
  var [aProducts, setAProducts] = useState([]);
  var toggleAgentProduct = (pid) => setAProducts(prev => prev.includes(pid) ? prev.filter(x=>x!==pid) : [...prev,pid]);
  ...
  var saveAgent=()=>{
    ...
    var assignedProds = products.filter(p=>aProducts.includes(p.id)).map(p=>({...p,initialStock:0,currentStock:0,totalDelivered:0}));
...
function StaffManager({staff,agents,addStaff,updateStaff,deleteStaff}) {
  var [sName,setSName]=useState(""); const [sUser,setSUser]=useState("");
  var [sPass,setSPass]=useState(""); var [sAgents,setSAgents]=useState([]); var [sProds,setSProds]=useState({});
  var [edit,setEdit]=useState(null);
  var [eN,setEN]=useState(""); var [eU,setEU]=useState(""); var [eP,setEP]=useState(""); var [eA,setEA]=useState([]); var [eProds,setEProds]=useState({});
  var [msg,setMsg]=useState(null);
  var flash=(text,type="good")=>{setMsg({text,type});setTimeout(()=>setMsg(null),3500);};

  var toggleAgent=(id,list,setList)=>{
  ...
  var save=()=>{
    ...
    addStaff({id:uid(),name:sName.trim(),username:sUser.trim(),password:sPass.trim(),role:"staff",assignedAgents:sAgents,assignedProducts:sProds});
    setSName("");setSUser("");setSPass("");setSAgents([]);setSProds({});
    flash("Staff member created!");
  };
  var saveEdit=id=>{
    if(!eN.trim()||!eU.trim()) return;
    updateStaff(id,s=>({...s,name:eN.trim(),username:eU.trim(),...(eP?{password:eP.trim()}:{}),assignedAgents:eA,assignedProducts:eProds}));
    setEdit(null); flash("Staff updated!");
  };

  var nonAdminStaff = staff.filter(s=>s.role!=="admin");
    ...
    <div>
      ...
      <div className="setup-grid" style={{display:"grid",gridTemplateColumns:"360px 1fr",gap:20,alignItems:"start"}}>
        ...
        <Card>
          ...
          <div style={{marginBottom:16}}>
            <label style={{fontSize:12,color:"#64748b",fontWeight:600,marginBottom:8,display:"block"}}>Assign Agents & Products</label>
            {agents.length===0&&<div style={{fontSize:12,color:"#334155"}}>No agents added yet. Add agents in Setup first.</div>}
            <div style={{display:"flex",flexDirection:"column",gap:8,maxHeight:320,overflowY:"auto"}}>
              {agents.map(function(a){
                var agentSel=sAgents.includes(a.id);
                var agProds=sProds[a.id]||[];
                return (
                  <div key={a.id} style={{borderRadius:8,border:"1px solid "+(agentSel?"#6366f1":"#1a2238"),background:"#07090f",overflow:"hidden"}}>
                    <label style={{display:"flex",alignItems:"center",gap:10,padding:"10px 12px",cursor:"pointer"}}>
                      <input type="checkbox" checked={agentSel} onChange={function(){
                        toggleAgent(a.id,sAgents,setSAgents);
                        if(agentSel) setSProds(function(v){var n={...v};delete n[a.id];return n;});
                      }} style={{accentColor:"#6366f1",width:15,height:15,flexShrink:0}}/>
                      <div style={{flex:1}}>
                        <div style={{fontSize:13,fontWeight:700,color:"#f1f5f9"}}>{a.agentName}</div>
                        <div style={{fontSize:11,color:"#475569"}}>{a.state}</div>
                      </div>
                      {agentSel&&<span style={{fontSize:10,color:"#818cf8",fontWeight:700,flexShrink:0}}>✓ Selected</span>}
                    </label>
                    {agentSel&&a.products&&a.products.length>0&&(
                      <div style={{padding:"8px 12px 10px",borderTop:"1px solid #1a2238",background:"#0a0c14"}}>
                        <div style={{fontSize:10,color:"#64748b",fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:".5px"}}>Products to manage:</div>
                        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:4}}>
                          {a.products.map(function(p){
                            var checked=agProds.includes(p.id);
                            return (
                              <label key={p.id} style={{display:"flex",alignItems:"center",gap:6,padding:"5px 8px",background:checked?"#6366f118":"transparent",borderRadius:6,border:"1px solid "+(checked?"#6366f1":"#1a2238"),cursor:"pointer"}}>
                                <input type="checkbox" checked={checked} onChange={function(){
                                  setSProds(function(v){
                                    var curr=v[a.id]||[];
                                    return {...v,[a.id]:checked?curr.filter(function(x){return x!==p.id;}):[...curr,p.id]};
                                  });
                                }} style={{accentColor:"#6366f1",width:12,height:12,flexShrink:0}}/>
                                <span style={{fontSize:11,color:checked?"#818cf8":"#64748b",fontWeight:checked?700:400}}>{p.name}</span>
                              </label>
                            );
                          })}
                        </div>
                  ...
                  <tr key={s.id} style={{background:"#0d1528"}}>
                    ...
                    <TD>
                      <div style={{display:"flex",flexDirection:"column",gap:6,maxHeight:260,overflowY:"auto"}}>
                        {agents.map(function(a){
                          var agSel=eA.includes(a.id);
                          var agProds=eProds[a.id]||[];
                          return (
                            <div key={a.id} style={{borderRadius:7,border:"1px solid "+(agSel?"#6366f1":"#1a2238"),background:"#07090f",overflow:"hidden"}}>
                              <label style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",cursor:"pointer"}}>
                                <input type="checkbox" checked={agSel} onChange={function(){
                                  toggleAgent(a.id,eA,setEA);
                                  if(agSel) setEProds(function(v){var n={...v};delete n[a.id];return n;});
                                }} style={{accentColor:"#6366f1",width:13,height:13,flexShrink:0}}/>
                                <div style={{flex:1}}>
                                  <div style={{fontSize:12,fontWeight:700,color:"#f1f5f9"}}>{a.agentName}</div>
                                  <div style={{fontSize:10,color:"#475569"}}>{a.state}</div>
                                </div>
                                {agSel&&<span style={{fontSize:10,color:"#818cf8",fontWeight:700,flexShrink:0}}>✓</span>}
                              </label>
                              {agSel&&a.products&&a.products.length>0&&(
                                <div style={{padding:"6px 10px 8px",borderTop:"1px solid #1a2238",background:"#0a0c14"}}>
                                  <div style={{fontSize:9,color:"#64748b",fontWeight:700,marginBottom:4,textTransform:"uppercase",letterSpacing:".5px"}}>Products:</div>
                                  <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:3}}>
                                    {a.products.map(function(p){
                                      var checked=agProds.includes(p.id);
                                      return (
                                        <label key={p.id} style={{display:"flex",alignItems:"center",gap:5,padding:"4px 6px",background:checked?"#6366f118":"transparent",borderRadius:5,border:"1px solid "+(checked?"#6366f1":"#1a2238"),cursor:"pointer"}}>
                                          <input type="checkbox" checked={checked} onChange={function(){
                                            setEProds(function(v){
                                              var curr=v[a.id]||[];
                                              return {...v,[a.id]:checked?curr.filter(function(x){return x!==p.id;}):[...curr,p.id]};
                                            });
                                          }} style={{accentColor:"#6366f1",width:11,height:11,flexShrink:0}}/>
                                          <span style={{fontSize:10,color:checked?"#818cf8":"#64748b",fontWeight:checked?700:400}}>{p.name}</span>
                                        </label>
                                      );
                                    })}
                                  </div>
                  ...
                  <tr key={s.id} className="rhov">
                    ...
                    <TD>
                      <div style={{display:"flex",gap:6}}>
                        <Btn small variant="ghost" onClick={()=>{setEdit(s.id);setEN(s.name);setEU(s.username);setEP("");setEA(s.assignedAgents||[]);setEProds(s.assignedProducts||{});}}>Edit</Btn>
                        <Btn small variant="danger" onClick={()=>deleteStaff(s.id, s.name, ()=>flash("Delete request sent to Main Admin!","warn"))}>Delete</Btn>
                      </div>
                    </TD>
                  </tr>
                ))}
...
function Approvals({pendingEntries,agents,approveEntry,rejectEntry}) {
  ...
  var list = pendingEntries.filter(e=>filter==="all"||e.status===filter).sort((a,b)=>new Date(b.submittedAt)-new Date(a.submittedAt));
  ...
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {list.map(e=>{
          var ag=agents.find(a=>a.id===e.agentId);
          var rate=e.oA>0?Math.round((e.oD/e.oA)*100):0;
          return(
                ...
                <div style={{background:"#07090f",borderRadius:8,padding:"10px 14px",border:"1px solid #151c2e"}}>
                  ...
                  <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                    {Object.entries(e.pQ||{}).filter(([,v])=>parseInt(v)>0).map(([pid,qty])=>{
                      var p=ag?.products.find(x=>x.id===pid);
...
function AgentsView({agents,products}) {
  ...
  var list=useMemo(()=>agents.filter(a=>{
    var q=search.toLowerCase();
    var ok=!q||a.state.toLowerCase().includes(q)||a.agentName.toLowerCase().includes(q);
    var r=deliveryRate(a);
    var rf=filter==="all"||(filter==="good"&&r>=80)||(filter==="avg"&&r>=60&&r<80)||(filter==="poor"&&r<60);
    return ok&&rf;
      ...
      <Card style={{padding:0,overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {list.map((a,i)=>{
                var r=deliveryRate(a); const stk=totalStock(a); const isExp=expanded===a.id;
                return [
...
function StockManager({agents,products,updateAgent}) {
  var [mode,setMode]=useState("init");
  var [agentId,setAgentId]=useState("");
  var [qtys,setQtys]=useState({});
        ...
        <Card>
          ...
          {agent&&<>
            ...
            {agent.products.map(p=>{
              var sug=suggestQty(agent,products).find(x=>x.id===p.id);
              return(
...
{mode==="init" && p.initialStock>0 && !unlocked[p.id] ? (
                    <div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                      <div style={{width:72,background:"#07090f",border:"1px solid #1a2238",borderRadius:8,color:"#10b981",padding:"10px 0",fontSize:14,fontWeight:700,textAlign:"center"}}>{p.initialStock}</div>
                      <button onClick={()=>setUnlocked(v=>({...v,[p.id]:true}))} style={{fontSize:10,color:"#f59e0b",background:"#f59e0b12",border:"1px solid #f59e0b30",borderRadius:5,padding:"2px 8px",cursor:"pointer"}}>🔓 Unlock</button>
                    </div>
                  ) : (
        ...
        <Card>
          ...
          {agents.filter(a=>a.stockHistory.length>0).length===0?<div style={{textAlign:"center",padding:"30px 0",color:"#1e2d40",fontSize:13}}>No dispatches yet.</div>
            :agents.filter(a=>a.stockHistory.length>0).slice(0,12).map(a=>{
              var last=a.stockHistory[a.stockHistory.length-1];
              return(
...
function WednesdayAlert({agents,products,totals}) {
  var critical=[...agents].filter(a=>totalStock(a)<20&&a.products.length>0).sort((a,b)=>totalStock(a)-totalStock(b));
  var low=[...agents].filter(a=>totalStock(a)>=20&&totalStock(a)<50&&a.products.length>0).sort((a,b)=>totalStock(a)-totalStock(b));
  var healthy=agents.filter(a=>totalStock(a)>=50);
  ...
  var generatePDF = () => {
    ...
    try {
      ...
      var dateStr = new Date().toLocaleDateString("en-NG",{weekday:"long",day:"numeric",month:"long",year:"numeric"});
      var margin = 14;
      ...
      var prodCols = products.map(p=>({header:p.name.length>14?p.sku:p.name, dataKey:p.id}));
      var head = [
        ["#","State","Agent Name",...products.map(p=>p.name.length>12?p.sku:p.name),"Total Stock","Rate","Status"]
      ];
      var rows = agents.map((a,i)=>{
        var r = deliveryRate(a);
        var stk = totalStock(a);
        var status = stk<20?"CRITICAL":stk<50?"LOW":"OK";
        return [
          String(i+1).padStart(2,"0"),
          a.state,
          a.agentName,
          ...products.map(p=>{
            var ap=a.products.find(x=>x.id===p.id);
      ...
      doc.autoTable({
        ...
        didParseCell(data){
          if(data.section==="body"){
            var lastCol = data.row.raw.length-1;
      ...
      products.forEach((p,pi)=>{
        ...
        var totalInit = agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.initialStock||0);},0);
        var totalDel  = agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.totalDelivered||0);},0);
        var totalLeft = agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.currentStock||0);},0);
        var pct = totalInit>0?Math.round((totalLeft/totalInit)*100):0;
        doc.setTextColor(100,116,139);
        doc.setFontSize(8);
        ...
        var agRows = agents
          .map(a=>{
            var ap=a.products.find(x=>x.id===p.id);
            var left=ap?.currentStock||0;
            var del=ap?.totalDelivered||0;
            var init=ap?.initialStock||0;
            var pctA=init>0?Math.round((left/init)*100):0;
            var status=left<5?"CRITICAL":left<15?"LOW":"OK";
            return [a.state, a.agentName, String(init), String(del), String(left), pctA+"%", status];
          })
          .sort((ra,rb)=>{
            var o={CRITICAL:0,LOW:1,OK:2};
      ...
      // ── SAVE ──
      var filename = "StockPulse_Thursday_"+new Date().toISOString().slice(0,10)+".pdf";
      doc.save(filename);
    } catch(e){
      console.error(e);
      ...
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse"}}>
          ...
          <tbody>
            {list.map(a=>{
              var r=deliveryRate(a);
              var sug=suggestQty(a,products).reduce((s,p)=>s+p.suggested,0);
              return(
                <tr key={a.id} className="rhov">
                  <TD><b style={{color:"#f1f5f9"}}>{a.agentName}</b></TD>
                  <TD>{a.state}</TD>
      ...
      {(critical.length>0||low.length>0)&&(
        <div className="sp-card" style={{background:"#0d1120",borderRadius:14,border:"1px solid #151c2e",padding:20}}>
          ...
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:12}}>
            {products.map(p=>{
              var total=[...critical,...low].reduce((s,a)=>{const sug=suggestQty(a,products).find(x=>x.id===p.id);return s+(sug?.suggested||0);},0);
              return(
...
function WeeklyReport({agents,products,totals}) {
  var ranked=[...agents].sort((a,b)=>deliveryRate(b)-deliveryRate(a));
  var totalStockDel=agents.reduce((s,a)=>s+a.products.reduce((ps,p)=>ps+p.totalDelivered,0),0);
    ...
    <div>
        ...
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse"}}>
            ...
            <tbody>
              {ranked.map((a,i)=>{
                var r=deliveryRate(a);
                var sDel=a.products.reduce((s,p)=>s+p.totalDelivered,0);
                var sLeft=totalStock(a);
                var sug=suggestQty(a,products).reduce((s,p)=>s+p.suggested,0);
                return(
      ...
      <Card>
        ...
        <div className="grid-3col" style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(150px,1fr))",gap:10}}>
          {products.map(p=>{
            var del=agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.totalDelivered||0);},0);
            var left=agents.reduce((s,a)=>{const ap=a.products.find(x=>x.id===p.id);return s+(ap?.currentStock||0);},0);
            return(
              <div key={p.id} style={{background:"#07090f",borderRadius:12,padding:16,border:"1px solid #151c2e"}}>
                <div style={{fontFamily:"'JetBrains Mono',monospace",fontSize:10,color:"#6366f1",marginBottom:3}}>{p.sku}</div>
...
/* ══ STAFF: ENTRY ═══════════════════════════════════════════ */
function StaffEntry({myAgents,products,submitEntry,session}) {
  var [agentId,setAgentId]=useState("");
  var [oA,setOA]=useState(""); const [oD,setOD]=useState("");
  var [pQ,setPQ]=useState({});
  var [done,setDone]=useState(false);
  var agent=myAgents.find(a=>a.id===agentId);
  var rate=oA&&oD?Math.round((parseInt(oD)||0)/Math.max(parseInt(oA)||1,1)*100):null;
          ...
          {agent&&<>
            ...
            <div style={{marginBottom:18}}>
              ...
              <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                {agent.products.filter(function(p){var ap=(session.assignedProducts&&session.assignedProducts[agent.id])||[];return ap.length===0||ap.includes(p.id);}).map(p=>(
...
function MySubmissions({pendingEntries,agents}) {
  var sorted=[...pendingEntries].sort((a,b)=>new Date(b.submittedAt)-new Date(a.submittedAt));
  return (
    <div>
...
function PasswordManager({staff, session, updateStaff}) {
  ...
  var [adminOld,  setAdminOld]  = useState("");
  var [adminNew,  setAdminNew]  = useState("");
  var [adminConf, setAdminConf] = useState("");
  ...
  var changeAdminPass = () => {
    var adminStaff = staff.find(s => s.id === session.id);
...
function ChangePassword({session, update, state}) {
  ...
  var save = () => {
    var me = state.staff.find(s => s.id === session.id);
    ...
    update(s=>({...s, staff:s.staff.map(st=>st.id===session.id?{...st,password:newPass.trim()}:st)}));
...
function ProductsOverview({ agents, products }) {
  ...
  var productStats = products.map(p => {
    var agentsWithProduct = agents.filter(a => a.products.some(x => x.id === p.id));
    var totalInitial   = agentsWithProduct.reduce((s,a) => { const ap=a.products.find(x=>x.id===p.id); return s+(ap?.initialStock||0); }, 0);
    var totalDelivered = agentsWithProduct.reduce((s,a) => { const ap=a.products.find(x=>x.id===p.id); return s+(ap?.totalDelivered||0); }, 0);
    var totalRemaining = agentsWithProduct.reduce((s,a) => { const ap=a.products.find(x=>x.id===p.id); return s+(ap?.currentStock||0); }, 0);
    var pct = totalInitial > 0 ? Math.round((totalRemaining/totalInitial)*100) : 0;
    var agentBreakdown = agentsWithProduct.map(a => {
      var ap = a.products.find(x => x.id===p.id);
      return {
  ...
  return (
    <div>
      ...
      {selected && (() => {
        var p = productStats.find(x=>x.id===selected);
        if(!p) return null;
        return (
              ...
              <table style={{width:"100%",borderCollapse:"collapse"}}>
                ...
                <tbody>
                  {p.agentBreakdown.map((a,i) => {
                    var usedPct = a.initial > 0 ? Math.round(((a.initial-a.remaining)/a.initial)*100) : 0;
                    var status = a.remaining < 5 ? "Critical" : a.remaining < 15 ? "Low" : "OK";
                    var statusColor = a.remaining < 5 ? "#ef4444" : a.remaining < 15 ? "#f59e0b" : "#10b981";
                    return (
...
/* ══ SUPER ADMIN MANAGER (main admin only) ══════════════════ */
function SuperAdminManager({staff, agents, addStaff, updateStaff, deleteStaff, session}) {
  var [sName,  setSName]  = useState("");
  var [sUser,  setSUser]  = useState("");
  var [sPass,  setSPass]  = useState("");
  var [msg,    setMsg]    = useState(null);
  var flash = (text,type="good") => { setMsg({text,type}); setTimeout(()=>setMsg(null),3500); };

  var save = () => {
    if(!sName.trim()||!sUser.trim()||!sPass.trim()) return flash("Fill in all fields","warn");
    if(sPass.length < 6) return flash("Password must be at least 6 characters","warn");
    if(staff.find(s=>s.username===sUser.trim())) return flash("Username already taken","warn");
    addStaff({ id:uid(), name:sName.trim(), username:sUser.trim(), password:sPass.trim(), role:"superadmin", assignedAgents:[] });
    setSName(""); setSUser(""); setSPass("");
    flash("Super Admin account created!");
  };

  var superAdmins = staff.filter(s => s.role==="superadmin" && s.id !== session.id);

  return (
    <div>
      <PageHeader title="Super Admin Management"
        sub="Create Super Admins who have full access but cannot delete without your approval"
        right={<div style={{background:"#6366f118",border:"1px solid #6366f130",color:"#818cf8",padding:"6px 14px",borderRadius:20,fontSize:12,fontWeight:700}}>👑 Main Admin Only</div>}
      />
      {msg && <InfoBox type={msg.type}>{msg.text}</InfoBox>}
      ...
      <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,marginBottom:24}}>
        <Card style={{border:"1px solid #6366f130"}}>
          <div style={{fontSize:14,fontWeight:800,color:"#f1f5f9",marginBottom:14}}>👑 Main Admin (You)</div>
          {["Full access to all features","Create & manage Super Admins","Delete agents, products & staff instantly","Approve or reject delete requests from Super Admins","Approve or reject staff daily entries","View all reports and analytics"].map(item=>(
            <div key={item} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",fontSize:13,color:"#94a3b8",borderBottom:"1px solid #0d1120"}}>
              <span style={{color:"#10b981",fontWeight:700}}>✓</span> {item}
            </div>
          ))}
        </Card>
        <Card style={{border:"1px solid #f59e0b30"}}>
          <div style={{fontSize:14,fontWeight:800,color:"#f1f5f9",marginBottom:14}}>⭐ Super Admin</div>
          {[
            {text:"Full access to all features", ok:true},
            {text:"Create & manage Super Admins", ok:false},
            {text:"Delete agents, products & staff", ok:false, note:"Requires your approval"},
            {text:"Approve or reject delete requests", ok:false},
            {text:"Approve or reject staff daily entries", ok:true},
            {text:"View all reports and analytics", ok:true},
          ].map(item=>(
            <div key={item.text} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",fontSize:13,color:"#94a3b8",borderBottom:"1px solid #0d1120"}}>
              <span style={{color:item.ok?"#10b981":"#ef4444",fontWeight:700}}>{item.ok?"✓":"✗"}</span>
              <span>{item.text}</span>
              {item.note && <span style={{fontSize:11,color:"#f59e0b",marginLeft:"auto"}}>({item.note})</span>}
            </div>
          ))}
        </Card>
      </div>
      ...
      <div className="grid-2col" style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:20,alignItems:"start"}}>
        {/* CREATE FORM */}
        <Card>
          <div style={{fontSize:15,fontWeight:800,color:"#f1f5f9",marginBottom:6}}>➕ Create Super Admin</div>
          <div style={{fontSize:12,color:"#64748b",marginBottom:18}}>Super Admins have full access but all delete actions require your approval first.</div>
          <div style={{display:"flex",flexDirection:"column",gap:12,marginBottom:20}}>
            <Inp label="Full Name" value={sName} onChange={e=>setSName(e.target.value)} placeholder="e.g. Chioma Okafor"/>
            <Inp label="Username" value={sUser} onChange={e=>setSUser(e.target.value)} placeholder="e.g. chioma.admin"/>
            <Inp label="Password" type="password" value={sPass} onChange={e=>setSPass(e.target.value)} placeholder="Min 6 characters"/>
          </div>
          {sPass.length>0 && (
            <div style={{marginBottom:14}}>
              <div style={{height:6,background:"#1a2238",borderRadius:3,overflow:"hidden"}}>
                <div style={{width:sPass.length<6?"20%":sPass.length<8?"50%":sPass.length<12?"75%":"100%",height:"100%",borderRadius:3,
                  background:sPass.length<6?"#ef4444":sPass.length<8?"#f59e0b":"#10b981",transition:"width .3s"}}/>
              </div>
            </div>
          )}
          <Btn onClick={save} style={{width:"100%"}}>Create Super Admin →</Btn>
        </Card>

        {/* SUPER ADMINS LIST */}
        <Card>
          <div style={{fontSize:15,fontWeight:800,color:"#f1f5f9",marginBottom:16}}>⭐ Current Super Admins ({superAdmins.length})</div>
          {superAdmins.length===0
            ? <EmptyState icon="⭐" title="No Super Admins yet" sub="Create your first Super Admin on the left."/>
            : superAdmins.map((s,i)=>(
              <div key={s.id} style={{background:"#07090f",borderRadius:10,padding:14,marginBottom:10,border:"1px solid #1a2238"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div>
                    <div style={{fontWeight:700,color:"#f1f5f9",fontSize:14}}>{s.name}</div>
                    <div style={{fontSize:12,color:"#6366f1",fontFamily:"'JetBrains Mono',monospace",marginTop:2}}>@{s.username}</div>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    <span style={{fontSize:11,background:"#f59e0b18",color:"#f59e0b",padding:"3px 10px",borderRadius:20,border:"1px solid #f59e0b30",fontWeight:700}}>⭐ Super Admin</span>
                    <Btn small variant="danger" onClick={()=>deleteStaff(s.id)}>Remove</Btn>
                  </div>
                </div>
              </div>
            ))
...
/* ══ DELETE REQUESTS ════════════════════════════════════════ */
function DeleteRequests({deleteRequests, approveDelete, rejectDelete, session, isSuperAdmin, agents, products, staff}) {
  var [filter, setFilter] = useState("pending");

  var list = [...(deleteRequests||[])].filter(r => filter==="all" || r.status===filter)
    .sort((a,b)=>new Date(b.requestedAt)-new Date(a.requestedAt));

  var typeIcon = {agent:"◉", product:"📦", staff:"👤"};
  var typeLabel = {agent:"Agent", product:"Product", staff:"Staff Member"};
  var typeColor = {agent:"#6366f1", product:"#f59e0b", staff:"#10b981"};

  return (
    <div>
      <PageHeader title="Delete Requests"
        sub={isSuperAdmin ? "Your delete requests — pending main admin approval" : "Review and approve delete requests from Super Admins"}
        right={<div style={{
          background: list.filter(r=>r.status==="pending").length>0 ? "#ef444418":"#10b98118",
          border:`1px solid ${list.filter(r=>r.status==="pending").length>0?"#ef444430":"#10b98130"}`,
          color: list.filter(r=>r.status==="pending").length>0?"#ef4444":"#10b981",
      ...
      {isSuperAdmin && (
        <InfoBox type="info">💡 When you click Delete on any agent, product or staff member — it sends a request to the Main Admin for approval. You cannot delete directly.</InfoBox>
      )}
      {!isSuperAdmin && list.filter(r=>r.status==="pending").length>0 && (
        <InfoBox type="warn">⚠️ You have <b>{list.filter(r=>r.status==="pending").length} pending delete request{list.filter(r=>r.status==="pending").length>1?"s":""}</b> from Super Admins waiting for your approval.</InfoBox>
      )}
      ...
      {list.length===0 && (
        <EmptyState icon="🗑" title="No delete requests" sub={filter==="pending"?"No pending requests — all clear!":"No requests found."}/>
      )}

      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {list.map(req => (
          <Card key={req.id} style={{border:`1px solid ${req.status==="pending"?"#f59e0b30":req.status==="approved"?"#10b98130":"#ef444430"}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:12}}>
              <div style={{display:"flex",alignItems:"center",gap:14}}>
                {/* Type badge */}
                <div style={{width:44,height:44,borderRadius:10,background:`${typeColor[req.type]}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:20,border:`1px solid ${typeColor[req.type]}30`,flexShrink:0}}>
                  {typeIcon[req.type]}
                </div>
                <div>
                  <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:3}}>
                    <span style={{fontSize:11,fontWeight:700,color:typeColor[req.type],background:`${typeColor[req.type]}18`,padding:"2px 8px",borderRadius:20,border:`1px solid ${typeColor[req.type]}30`,textTransform:"uppercase",letterSpacing:.5}}>
                      {typeLabel[req.type]}
                    </span>
                    <span style={{fontSize:15,fontWeight:800,color:"#f1f5f9"}}>{req.targetName}</span>
                  </div>
                  {req.type==="staff" && (() => { var m=staff.find(s=>s.id===req.targetId); return m?(<div style={{background:"#07090f",borderRadius:8,padding:"6px 10px",marginBottom:4,fontSize:12,color:"#94a3b8"}}><b style={{color:"#f1f5f9"}}>@{m.username}</b> · {m.role}</div>):null; })()}
                  {req.type==="agent" && (() => { var a=agents.find(x=>x.id===req.targetId); return a?(<div style={{background:"#07090f",borderRadius:8,padding:"6px 10px",marginBottom:4,fontSize:12,color:"#94a3b8"}}><b style={{color:"#f1f5f9"}}>{a.state}</b> · {a.phone||"no phone"}</div>):null; })()}
                  {req.type==="product" && (() => { var p=products.find(x=>x.id===req.targetId); return p?(<div style={{background:"#07090f",borderRadius:8,padding:"6px 10px",marginBottom:4,fontSize:12,color:"#94a3b8"}}>SKU: <b style={{color:"#f1f5f9"}}>{p.sku}</b></div>):null; })()}
                  <div style={{fontSize:12,color:"#475569"}}>
                    Requested by <b style={{color:"#818cf8"}}>{req.requestedByName}</b> · {new Date(req.requestedAt).toLocaleString("en-NG")}
                  </div>
                </div>
              </div>

              <div style={{display:"flex",alignItems:"center",gap:10}}>
                <span style={{fontSize:12,fontWeight:700,padding:"5px 14px",borderRadius:20,
                  background:req.status==="pending"?"#f59e0b18":req.status==="approved"?"#10b98118":"#ef444418",
                  color:req.status==="pending"?"#f59e0b":req.status==="approved"?"#10b981":"#ef4444",
                  border:`1px solid ${req.status==="pending"?"#f59e0b30":req.status==="approved"?"#10b98130":"#ef444430"}`}}>
                  {req.status==="pending"?"⏳ Pending":req.status==="approved"?"✅ Approved":"❌ Rejected"}
                </span>
              </div>
            </div>

            {/* Action buttons — only main admin can approve/reject */}
            {req.status==="pending" && !isSuperAdmin && (
              <div style={{display:"flex",gap:10,marginTop:14,paddingTop:14,borderTop:"1px solid #0d1120"}}>
                <Btn variant="success" onClick={()=>approveDelete(req.id)} style={{flex:1}}>
                  ✅ Approve — Delete {typeLabel[req.type]}
                </Btn>
                <Btn variant="danger" onClick={()=>rejectDelete(req.id)} style={{flex:1}}>
                  ❌ Reject Request
                </Btn>
              </div>
            )}
            ...
            {req.status==="pending" && isSuperAdmin && (
              <div style={{marginTop:12,background:"#f59e0b10",border:"1px solid #f59e0b25",borderRadius:8,padding:"10px 14px",fontSize:12,color:"#f59e0b"}}>
